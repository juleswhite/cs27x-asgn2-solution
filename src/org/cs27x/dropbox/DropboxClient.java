 ...
 * into changes on the local file system. The DropboxClient is also responsible
 * for translating FileEvents into DropboxCmds and propagating them to other
 * clients via the DropboxTransport.
 * 
 * 
 * @author jules
 *
...
public class DropboxClient implements FileEventListener, DropboxCmdListener {
	...
	/**
	 * This method should translate FileEvents into DropboxCmds and
	 * publish them to the DropboxTransport as follows:
	 * 
	 * 1. ENTRY_CREATE --> ADD
	 * 2. ENTRY_MODIFY --> UPDATE
	 * 3. ENTRY_DELETE --> REMOVE
	 * 
	 * Create and modify cmds should be created with the data contained
	 * in the FileEvent.
	 * 
	 * DropboxCmds should only be created if the FileSystemState indicates
	...
	public void handleEvent(FileEvent rawEvt) {
		...
		final Path sharedPath = fileManager_.ensureRelative(rawEvt.getFile());
		final FileEvent evt = new FileEvent(rawEvt.getEventType(), sharedPath,
				rawEvt.getData());

		if (fileSystemState_.updateState(evt)) {

			final OpCode code = EVENT_TYPE_TO_OPCODES.get(evt.getEventType());
	...
	/**
	 * This method should add, update, and delete
	 * files in response to incoming DropboxCmds.
	 * 
	 * Add/Update cmds should trigger a write on the
	 * FileManager with the data in the cmd.
	 * 
	 * Remove cmds should trigger a delete on the
	 * FileManager
	 * 
	 * All Paths should be resolved using the FileManager
	 * before invoking any file management operations.
	 * 
	 * DropboxCmds should only be executed if the FileSystemState
	 * indicates that they should be propagated.
	 */
	@Override
